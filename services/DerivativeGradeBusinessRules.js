...
class DerivativeGradeBusinessRules {
  ...
  static validateWingFillet(derivative, grade, speciesType) {
    ...
    if (BLOCKED_GRADES.includes(grade)) {
      return {
        ...
        reason: 'Wing fillets degrade faster than center cuts - quality constraints',
  ...
  static validateTuna(derivative, grade, storageTemp) {
    const rules = {
      ...
      SAKU: {
        ...
        temperature_requirement: {
          ...
          reason: 'Sashimi buyers expect frozen storage at -60°C or chilled sashimi protocol'
        }
      },
      STEAK: {
        allowed: ['A', 'B', 'C'],
    ...
    if (derivative === 'SAKU' && grade === 'A' && storageTemp) {
      if (storageTemp > -60) {
        return {
          valid: false,
          error: 'Grade A SAKU requires storage at -60°C or sashimi chilled protocol',
  ...
  static validateGradeCWhole(derivative, grade, speciesType) {
    ...
    const ALLOWED_WHOLE_C = ['ROUND_FISH', 'FLAT_FISH', 'SQUID', 'OCTOPUS_SMALL'];
