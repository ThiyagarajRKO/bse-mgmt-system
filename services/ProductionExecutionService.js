...
class ProductionExecutionService {
  ...
  async recordProduction(data) {
    ...
    try {
      ...
      if (!order) {
        throw new Error(
          `Production order not found: ${data.production_order_id}`
        );
      }
      ...
      if (!prodDerivative) {
        throw new Error(
          `Production derivative not found: ${data.production_derivative_id}`
        );
      }
      ...
      if (!sizeRecord) {
        throw new Error(
          `Size code not found in size_master: ${data.size_code}`
        );
      }
      ...
      const yieldPercent =
        (data.actual_quantity_kg / prodDerivative.expected_quantity_kg) * 100;
      const yieldTolerance = this.config.yieldTolerance || 5; // ±5%

      if (yieldPercent > 100 + yieldTolerance) {
        throw new Error(
          `Yield exceeds maximum tolerance. Actual: ${yieldPercent.toFixed(
            2
          )}%, Max: ${100 + yieldTolerance}%`
        );
      }
  ...
  async getOrderProductionSummary(orderId) {
    ...
    const totalProduced = outputs.reduce(
      (sum, o) => sum + (o.actual_quantity_kg || 0),
      0
    );
    const totalExpected = outputs.reduce(
      (sum, o) => sum + (o.expected_quantity_kg || 0),
      0
    );
    const totalCost = outputs.reduce(
      (sum, o) => sum + (o.cost_allocated || 0),
      0
    );
