...
class DerivativeAllocationService {
  constructor(models, derivativeGradeRulesService, productionYieldService, config = {}) {
    this.models = models;
  ...
  async allocateDerivatives(data) {
    ...
    try {
      ...
      const order = await this._validateOrder(data.production_order_id, transaction);
      const rawIssue = await this._validateRawIssue(data.production_order_id, transaction);

      if (!rawIssue) {
        throw new Error('Raw material must be issued before allocating derivatives');
      }
      ...
      for (const derivData of data.derivatives) {
        ...
        const expectedQty = (rawIssue.issued_quantity_kg * derivData.planned_percentage / 100) 
      ...
      const totalExpected = derivativeList.reduce((sum, d) => sum + d.expected_quantity_kg, 0);
      ...
      let hasAutoMinc = derivativeList.some(d => d.derivative_name === 'MINCE');
   ...
   */
  async _validateDerivativeAllowed(derivativeId, speciesId, grade, transaction) {
    const derivative = await this.models.derivative_master.findByPk(derivativeId, { transaction });
    ...
    if (this.derivativeGradeRulesService) {
      const allowed = await this.derivativeGradeRulesService.isDerivativeAllowed(
        speciesId,
        derivativeId,
        grade,
        { transaction }
  ...
  _validatePercentageSum(derivatives) {
    ...
    const sum = manualDerivs.reduce((total, d) => total + parseFloat(d.planned_percentage || 0), 0);
    const tolerance = 0.01;  // Allow 0.01% rounding difference

    if (Math.abs(sum - 100) > tolerance) {
      throw new Error(
        `Derivative percentages must sum to 100%. Current sum: ${sum.toFixed(2)}%`
      );
    }
  }
}
