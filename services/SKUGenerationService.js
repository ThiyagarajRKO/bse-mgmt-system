...
class SKUGenerationService {
  ...
  async generateSKU(data) {
    ...
    try {
      ...
      const derivative = await this._validateDerivative(data.derivative_id, transaction);
      ...
      const skuCode = this._generateSKUCode(
        species.species_code || species.species_name.substring(0, 3).toUpperCase(),
        derivative.derivative_code || derivative.derivative_name.substring(0, 3).toUpperCase(),
        data.grade,
        data.size_code,
        packSize
      );
  ...
  async generateAndLinkSKUForOutput(data) {
    ...
    try {
      ...
      if (!output) {
        throw new Error(`Production output not found: ${data.production_output_id}`);
      }
      ...
      return await this.models.production_outputs.findByPk(data.production_output_id, {
        include: [
          {
  ...
  async _validateSpecies(speciesId, transaction) {
    const species = await this.models.species_master.findByPk(speciesId, { transaction });
  ...
  async getOrderSKUs(orderId) {
    const outputs = await this.models.production_outputs.findAll({
      ...
      attributes: ['id', 'sku_code', 'actual_quantity_kg', 'actual_grade', 'size_code']
    });
