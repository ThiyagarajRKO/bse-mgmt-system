...
class ProductionOrderController {
  ...
  async allocateDerivatives(request, reply) {
    try {
      ...
      const derivatives = await this.derivativeAllocationService.allocateDerivatives({
        production_order_id: orderId,
      ...
      return reply.status(200).send({
        ...
        data: {
          ...
          total_expected_kg: expectedYields.reduce((sum, d) => sum + d.expected_quantity_kg, 0),
  ...
  async allocateCost(request, reply) {
    try {
      ...
      const output = await this.productionExecutionService.allocateCosts({
        ...
        processing_cost_share: parseFloat(request.body.processing_cost_share || 0),
        packaging_cost_share: parseFloat(request.body.packaging_cost_share || 0),
        allocated_by: userId
      });
  ...
  async getExpectedYield(request, reply) {
    try {
      ...
      const derivatives = await this.derivativeAllocationService.getDerivativesByOrderId(orderId);
  ...
  async getCostAllocation(request, reply) {
    try {
      ...
      const outputs = await this.productionExecutionService.getOutputsByOrderId(orderId);
      ...
      const totalCost = costBreakdown.reduce((sum, c) => sum + (c.cost_allocated || 0), 0);
      ...
      return reply.status(200).send({
        ...
        data: {
          ...
          summary: {
            ...
            average_cost_per_kg: totalCost / outputs.reduce((sum, o) => sum + o.actual_quantity_kg, 0)
          }
        }
      });
    } catch (error) {
  ...
  async getProductionSummary(request, reply) {
    try {
      ...
      const summary = await this.productionExecutionService.getOrderProductionSummary(orderId);
  ...
  _handleError(reply, error) {
    ...
    // State errors
    if (error.message.includes('must be in') || error.message.includes('status')) {
