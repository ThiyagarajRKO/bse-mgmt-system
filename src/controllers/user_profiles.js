...
export const GetAll = async ({
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (type != "admin") {
        ...
        where[Op.and] = [
          models.Sequelize.literal(
            `SELECT CASE WHEN COUNT(blocked_users.id) > 0 THEN false ELSE true END from blocked_users WHERE blocked_users.blocked_user_id = "UserProfiles".id and blocked_users.created_by = '${profile_id}'`
          ),
        ];
      }
      ...
      const user_data = await models.UserProfiles.findAndCountAll({
        // required: true,
        attributes: [
          "id",
          "full_name",
          ...
          [
            models.sequelize.literal(
              `(SELECT COUNT(user_follows.id) FROM user_follows WHERE user_follows.source_user_id = "UserProfiles".id and user_follows.is_active = true)`
            ),
            "total_followers",
          ],
          [
            models.sequelize.literal(
              `(SELECT COUNT(user_follows.id) FROM user_follows WHERE user_follows.created_by = "UserProfiles".id and user_follows.is_active = true)`
            ),
            "total_following",
          ],
          "is_active",
      ...
      delete user_data?.dataValues?.User;
      resolve(user_data);
    } catch (err) {
      reject(err);
    }
...
export const GetUserWithStatusByProfileId = ({
  id,
  username,
  current_profile_id,
}) => {
    ...
    try {
      ...
      const user = await models.UserProfiles.findOne({
        ...
        attributes: [
          ...
          [
            models.Sequelize.literal(
              `(SELECT CASE WHEN COUNT(blocked_users.id) > 0 THEN true ELSE false END from blocked_users WHERE blocked_users.blocked_user_id = "UserProfiles".id and blocked_users.created_by = '${current_profile_id}')`
            ),
            "is_this_user_blocked",
          ],
          [
            models.Sequelize.literal(
              `(SELECT COUNT(posts.id) from posts JOIN posts_likes ON posts_likes.post_id = posts.id and posts_likes.is_active = true WHERE posts.created_by = '${
                id || current_profile_id
              }' and posts.is_active = true and posts.is_banned = false)`
            ),
            "total_stars",
          ],
          "avatar_view_type",
        ...
        include: [
          ...
          },
          {
            ...
            where: {
              is_active: true,
              source_user_id: current_profile_id,
            },
          },
        ],
        where,
        // group: ["UserProfiles.id", "User.id"],
      });
      resolve(user);
    } catch (err) {
      reject(err);
