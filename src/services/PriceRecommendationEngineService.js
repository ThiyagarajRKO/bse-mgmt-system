...
class PriceRecommendationEngineService {
    ...
    try {
      ...
      for (const skuData of profitabilityData) {
        ...
        if (recommendation) {
          ...
          results.analysis.marginAtRisk += Math.abs(
            recommendation.expected_margin_impact
          );
        }
      }
      ...
      results.recommendations.sort((a, b) => {
        ...
        return (
          Math.abs(b.expected_margin_impact) -
          Math.abs(a.expected_margin_impact)
        );
      });
  ...
  async analyzeSKUForRecommendation(skuData) {
    ...
    const yieldAlert = validRecommendations.find(
      (r) => r.recommendation_type === "YIELD_ALERT"
    );
    const priceIncrease = validRecommendations.find(
      (r) => r.recommendation_type === "PRICE_INCREASE"
    );
  ...
  async analyzePriceIncrease(skuData, currentPrice) {
    const { avg_margin_pct, trend_yield_change, volume_sold_kg, sku_code } =
      skuData;

    // Conditions for price increase recommendation
    const marginTooLow = avg_margin_pct < this.TARGET_MARGIN_PCT;
    const yieldDeclining = trend_yield_change > 2.0; // Yield loss increased more than 2% (yield declined)
    ...
    const expectedMarginImpact =
      volume_sold_kg *
      (recommendedPrice - currentPrice) *
      (this.TARGET_MARGIN_PCT / 100);
  ...
  async analyzeDiscountRisk(skuData, currentPrice) {
    ...
    const riskLevel =
      discountData.avgDiscountPct > marginBuffer ? "HIGH" : "MEDIUM";
    ...
    return {
      ...
      expected_margin_impact: -(
        (discountData.avgDiscountPct * currentPrice * skuData.volume_sold_kg) /
        100
      ),
  ...
  async analyzeStopSell(skuData, currentPrice) {
    ...
    return {
      ...
      expected_margin_impact:
        skuData.volume_sold_kg * (currentPrice * 0.25) * (avg_margin_pct / 100),
  ...
  async getProfitabilityAnalysisData() {
    ...
    const data = await ProfitabilityFact.findAll({
      attributes: [
        ...
        [
          Sequelize.fn(
            "SUM",
            Sequelize.literal(
              "CASE WHEN gross_margin > 0 THEN gross_margin ELSE 0 END"
            )
          ),
          "total_margin",
        ],
        [
          Sequelize.fn(
            "COUNT",
            Sequelize.literal("DISTINCT DATE_TRUNC('month', posting_date)")
          ),
          "active_months",
        ],
      ],
      ...
      group: ["sku_code", "market", "product_id", "species_id"],
      having: Sequelize.literal("COUNT(*) >= 10"), // Minimum data points for reliable analysis
      raw: true,
    });

    // Enrich with trend data
    for (const item of data) {
      const trends = await this.calculateTrends(item.sku_code, item.market);
      item.trend_yield_change = trends.yield_change || 0;
      item.trend_margin_change = trends.margin_change || 0;
      item.consecutive_low_margin_months =
        await this.getConsecutiveLowMarginMonths(item.sku_code, item.market);
      item.volume_sold_kg = await this.getVolumeSold(
        item.sku_code,
        item.market
      );
      // Keep trend_data for debugging
      item.trend_data = trends;
    }
  ...
  async calculateTrends(skuCode, market) {
    ...
    const trendData = await ProfitabilityFact.findAll({
      attributes: [
        [
          Sequelize.fn("DATE_TRUNC", "month", Sequelize.col("posting_date")),
          "month",
        ],
      ...
      where: {
        ...
        posting_date: {
          [Op.gte]: new Date(
            Date.now() - this.ROLLING_WINDOW_DAYS * 24 * 60 * 60 * 1000
          ),
        },
      },
      group: [
  ...
  async getCFODashboard() {
    ...
    const summary = await PriceRecommendation.findAll({
      attributes: [
        ...
        "status",
        [
          Sequelize.fn("SUM", Sequelize.col("expected_margin_impact")),
          "total_margin_impact",
        ],
    ...
    const totalRecommendations = summary.reduce(
      (sum, item) => sum + parseInt(item.count),
      0
    );

    const criticalRecommendations = summary
      .filter((item) => item.priority === "CRITICAL")
      .reduce((sum, item) => sum + parseInt(item.count), 0);

    // Calculate average confidence from top recommendations
    const topRecs = await this.getTopRecommendations(10);
    const avgConfidence =
      topRecs.length > 0
        ? (topRecs.reduce(
            (sum, rec) => sum + parseFloat(rec.confidence_score || 0),
            0
          ) /
            topRecs.length) *
          100
    ...
    return {
      ...
      marginAtRisk: summary.reduce(
        (sum, item) => sum + parseFloat(item.total_margin_impact || 0),
        0
      ),
      ...
      recommendationsByType: summary.reduce((acc, item) => {
        acc[item.recommendation_type] =
          (acc[item.recommendation_type] || 0) + parseInt(item.count);
        return acc;
      }, {}),
