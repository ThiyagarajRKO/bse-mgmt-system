...
import { GetChatPresence, GetChatUser } from "../../../../utils/chat/users";
...
export const GetProfileInfo = (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let { user: chat_user_data } = await GetChatUser({
        userId: session?.chat_user_id,
        authToken: session?.chat_token,
        userIds: user_result?.chat_user_id,
        isRoomsRequired: true,
      });

      let unReadRoomCount = chat_user_data?.rooms?.filter(
        (room) => room.rid != "GENERAL" && room.unread > 0
      ).length;
      ...
      let output_data = {
        ...
        chat_user_status: chat_user_data?.user?.status || "offline",
        chat_unread_rooms: unReadRoomCount,
        profile_main_info: {
          user_id: user_result?.dataValues?.id || "",
