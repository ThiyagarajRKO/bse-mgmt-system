  ...
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      if (columnsToAdd.length > 0) {
        console.log(
          `Adding ${columnsToAdd.length} missing columns to product_gst_mapping...`,
        );
        await Promise.all(columnsToAdd);
        console.log("✓ All missing columns added successfully");
      } else {
        console.log(
          "✓ All required columns already exist in product_gst_mapping",
        );
      }
    ...
    try {
      ...
      for (const column of columnsToRemove) {
        if (tableDescription[column]) {
          removals.push(
            queryInterface.removeColumn("product_gst_mapping", column, {
              transaction,
            }),
          );
        }
      ...
      if (removals.length > 0) {
        console.log(
          `Removing ${removals.length} columns from product_gst_mapping...`,
        );
