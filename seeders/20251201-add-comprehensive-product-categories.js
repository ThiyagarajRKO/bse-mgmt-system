...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const categories = [
      ...
      // SHARK FORMS
      { name: "Whole", speciesKeyword: "Shark" },
      { name: "Cut Piece", speciesKeyword: "Shark" },
      { name: "Skinless Fillet", speciesKeyword: "Shark" },
      { name: "Boneless Fillet", speciesKeyword: "Shark" },
      { name: "Flake Cut", speciesKeyword: "Shark" },

      // RAY FORMS
      { name: "Whole", speciesKeyword: "Ray" },
      { name: "Wing Fillet", speciesKeyword: "Ray" },
      { name: "Wing Cut", speciesKeyword: "Ray" },
      { name: "Boneless Fillet", speciesKeyword: "Ray" },
    ...
    if (categoryRows.length > 0) {
      await queryInterface.bulkInsert("product_category_master", categoryRows, {
  ...
  down: async (queryInterface, Sequelize) => {
    // Remove shark and ray categories added by this seeder
    await queryInterface.sequelize.query(
      `DELETE FROM product_category_master 
       WHERE product_category IN ('Whole', 'Cut Piece', 'Skinless Fillet', 'Boneless Fillet', 'Flake Cut', 'Wing Fillet', 'Wing Cut')
       AND species_master_id IN (
         SELECT id FROM species_master 
         WHERE species_name LIKE '%Shark%' OR species_name LIKE '%Ray%'
       )`
    );
  },
};
