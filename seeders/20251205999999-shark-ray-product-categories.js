...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    try {
      ...
      const productCategories = [
        ...
        {
          species_code: "SH001",
          product_category: "Cut Piece",
          grades: ["Cut Piece"],
          sizes: ["Small (30–50 cm)", "Medium (50–70 cm)", "Large (70–90 cm)"],
        },
        {
          species_code: "SH001",
          product_category: "Skinless Fillet",
        ...
        {
          species_code: "SH003",
          product_category: "Boneless Fillet",
      ...
      // Clean up old entries with species names in product_category field
      console.log(
        "🧹 Cleaning up old product categories with species names..."
      );
      await queryInterface.sequelize.query(
        `DELETE FROM product_category_to_grade_master 
         WHERE product_category_master_id IN (
           ...
           WHERE species_master_id IN (
             SELECT id FROM species_master 
             WHERE species_code LIKE 'SH%' OR species_code LIKE 'RY%'
           )
           AND (
             product_category LIKE '%Spiny Dogfish%' 
             OR product_category LIKE '%Hammerhead Shark%' 
      ...
      console.log("✅ Cleaned up old product categories with species names");
  ...
  down: async (queryInterface) => {
    try {
      ...
      // Delete product categories for shark and ray species
      await queryInterface.sequelize.query(
        `DELETE FROM product_category_master 
         WHERE species_master_id IN (
           SELECT id FROM species_master 
