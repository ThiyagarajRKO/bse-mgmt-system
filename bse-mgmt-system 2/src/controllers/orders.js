...
export const GetAllocationData = ({ start, length, search }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Note: We cache per search query, not per pagination
      const allocationCacheKey = cache.generateKey("allocation", {
        search: search || "all",
      });

      // Check if we have cached data
      if (cache.has(allocationCacheKey)) {
        console.log(
          `[AllocationData] Using cached data for search: ${search || "all"}`,
        );
        const cachedData = cache.get(allocationCacheKey);
        // Apply pagination to cached data
        const paginatedData = {
          rows: cachedData.rows.slice(start, start + length),
      ...
      const processedOrders = await Promise.all(
        orders.rows.map(async (order) => {
          ...
          // Check if there are ANY ALLOCATED SalesAllocations (means purchase was approved)
          const hasAllocatedSalesAllocations = salesAllocations.some(
            (alloc) => alloc.allocation_status === "ALLOCATED",
          );

          // An allocation is only pending purchase if:
          // 1. There are allocations with PENDING_PURCHASE status, AND
          // 2. There are NO ALLOCATED allocations (no approved purchases yet)
          const hasPendingPurchase =
            hasSalesAllocationPendingPurchase && !hasAllocatedSalesAllocations;
      ...
      const cachingKey = cache.generateKey("allocation", {
        search: search || "all",
      });
      cache.set(
        cachingKey,
      ...
      // Apply pagination
      const paginatedData = {
        rows: processedOrders.slice(start, start + length),
        count: orders.count,
      };
